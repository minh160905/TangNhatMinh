...
export default function ChildCommentsPage() {
  ...
  // Fetch report for header metadata
  const { data: report } = useQuery({
    queryKey: ['child-report', selectedStudentId, semester],
    queryFn: () => studentApi.getReport({ studentId: selectedStudentId, semester }).then(r => r.data.data),
    enabled: !!selectedStudentId,
  });

  // Fetch comments
  const { data: comments = [], isLoading: commentsLoading, error: commentsError } = useQuery({
    queryKey: ['child-comments', selectedStudentId, semester],
    queryFn: () => commentsApi.getAll({ student_id: selectedStudentId, semester }).then(r => r.data.data),
    enabled: !!selectedStudentId,
  });

  // Fetch conducts
  const { data: conducts = [], isLoading: conductsLoading, error: conductsError } = useQuery({
    queryKey: ['child-conducts', selectedStudentId, semester],
    queryFn: () => conductsApi.getByClass({ student_id: selectedStudentId, semester }).then(r => r.data.data),
    enabled: !!selectedStudentId,
  });
  ...
  const isLoading = childLoading || commentsLoading || conductsLoading;
  ...
  return (
    <div>
      <div className="page-header">
        <h1>💬 Nhận xét & Hạnh kiểm của con</h1>
        <p>Theo dõi nhận xét học tập và đánh giá hạnh kiểm từ giáo viên dành cho con em của bạn</p>
      </div>
      ...
      {children.length === 0 ? (
        ...
        <>
          {/* Child selection & Semester selector */}
          ...
          {!selectedStudentId ? (
            <div style={{ textAlign: 'center', padding: 60, color: '#6B7280' }}>
              ...
              <div style={{ marginTop: 12 }}>Chọn con em để xem nhận xét</div>
            </div>
          ) : isLoading ? (
